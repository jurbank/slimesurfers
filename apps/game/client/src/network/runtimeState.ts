...
function cloneRuntimeState(state: RuntimePlayerState): RuntimePlayerState {
  return {
    ...
    gravityAnchorPlanetId: state.gravityAnchorPlanetId ?? "",
    loadedPadId: state.loadedPadId ?? "",
    padLoadProgress: state.padLoadProgress ?? 0,
    padChargeProgress: state.padChargeProgress ?? 0,
    padCancelArmed: state.padCancelArmed ?? false,
...
export function snapshotToRuntimeState(snapshot: PlayerSnapshot): RuntimePlayerState {
  return {
    ...
    gravityAnchorPlanetId: snapshot.gravityAnchorPlanetId ?? snapshot.planetId ?? "",
    loadedPadId: snapshot.loadedPadId ?? "",
    padLoadProgress: snapshot.padLoadProgress ?? 0,
    padChargeProgress: snapshot.padChargeProgress ?? 0,
    padCancelArmed: snapshot.padCancelArmed ?? false,
...
function interpolateState(
  ...
  return {
    ...
    gravityAnchorPlanetId: newer.gravityAnchorPlanetId ?? "",
    loadedPadId: newer.loadedPadId ?? "",
    padLoadProgress: lerp(older.padLoadProgress ?? 0, newer.padLoadProgress ?? 0, t),
    padChargeProgress: lerp(older.padChargeProgress ?? 0, newer.padChargeProgress ?? 0, t),
    padCancelArmed: newer.padCancelArmed ?? false,
