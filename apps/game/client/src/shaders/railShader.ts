...
export const railFragmentShader = `
  ...
  void main() {
    ...
    slimeColor = mix(uSlimeNodes[index], uSlimeNodes[nextIndex], f);

    // If the slime color is white (0xffffff), it's unslimed.
    // We use a small epsilon for comparison.
    bool isSlimed = length(slimeColor - vec3(1.0, 1.0, 1.0)) > 0.01;

    vec3 finalColor = isSlimed ? slimeColor : uBaseColor;
